...
import com.google.maps.android.SphericalUtil
 ...
 class ParkingCommuneAdapter(var context: FragmentActivity) :
    RecyclerView.Adapter<ParkingCommuneAdapter.ParkingHolder>() {

    var data = mutableListOf<Parking>()
    var latitude_destination: Double = 0.0
    var longitude_destination: Double = 0.0
    ...
    fun setPostion(latitude_actuelle: Double, longitude_actuelle: Double, vitesse: Double) {
        this.latitude_destination = latitude_actuelle
        this.longitude_destination = longitude_actuelle
        this.vitesse = vitesse
        notifyDataSetChanged()
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ParkingCommuneAdapter.ParkingHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.layout_parking, parent, false)

        return ParkingCommuneAdapter.ParkingHolder(view)
    }
    ...
    private fun bind(holder: ParkingHolder, parking: Parking) {
        holder.apply {
            ...
            val pos2 = LatLng(latitude_destination, longitude_destination)
            var distance = SphericalUtil.computeDistanceBetween(pos1, pos2);
            if (latitude_destination == 0.0 || longitude_destination == 0.0) {
    ...
    override fun onBindViewHolder(holder: ParkingCommuneAdapter.ParkingHolder, position: Int) {
